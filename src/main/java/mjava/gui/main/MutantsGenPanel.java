...
public class MutantsGenPanel extends JPanel {
    ...
    private GenMutantsMain parent_frame;

    private JButton runB = new JButton("Generate");

    private JTable fileTable = new JTable();
    private JButton fileNoneB = new JButton("None");
    private JButton fileAllB = new JButton("All");

    private JTable traditionalOpTable = new JTable();
    private JButton traditionalNoneB = new JButton("None");
    private JButton traditionalAllB = new JButton("All");
    ...
    private void jbInit() {
        ...
        // RIGHT part
        rightPanel.setPreferredSize(new Dimension(150, 660));
        rightPanel.setMaximumSize(new Dimension(150, 660));
        rightPanel.setLayout(new BoxLayout(rightPanel, BoxLayout.PAGE_AXIS));
        rightPanel.setBorder(new EtchedBorder());


        JTextField titleOP = new JTextField(" Mutation Operators");
    ...
    private void initTripleColumnWidth(JTable table, AbstractTableModel model,
    ...
    private void fileNoneB_mouseClicked(MouseEvent e) {
        FileTableModel table = (FileTableModel) fileTable.getModel();
    ...
    private void runB_mouseClicked(MouseEvent e) {
        //disable the button
        ...
        generateMutants(file_list,traditional_ops);
        runB.setEnabled(true);
        parent_frame.tvPanel.refreshEnv();
    }

    public static void generateMutants(String[] file_list,String[] traditional_ops){
        ...
        MutantsGenerator.initTypeCheckingEnv();
        ...
        float totalSeconds = (System.currentTimeMillis() - startMilliseconds)/1000.0F;
        System.out.println("------------------------------------------------------------------");
        System.out.println("All files are handled. Execution Time:"+String.format("%.2f",totalSeconds)+"s");
    }
    ...
    public static void setMutationSystemPathFor(String file_name) {
        try {
            ...
            if (temp.startsWith(MutationSystem.SRC_PATH)){
                temp = temp.replace(MutationSystem.SRC_PATH,"");
            }
            temp = temp.replace('/', '.');
            temp = temp.replace('\\', '.');
            int separator_index = temp.indexOf(".");
            if (separator_index == 0) {
                temp = temp.substring(separator_index + 1, temp.length());
            }
            // e.g. com.phikal.regex.Activities.GameActivity
    ...
    private void traditionalAllB_mouseClicked(MouseEvent e) {
    ...
    private void traditionalNoneB_mouseClicked(MouseEvent e) {
        TMOTableModel table = (TMOTableModel) traditionalOpTable.getModel();
        table.setAllSelectValue(false);
        traditionalOpTable.setModel(table);
        traditionalOpTable.repaint();
    ...
    private void initFileColumnSizes(JTable table, AbstractTableModel model) {
        initTripleColumnWidth(table, model, 30, 700, 80);
    }

    public void updateFileTable(){ // update file table
        FileTableModel fTableModel = new FileTableModel(MutationSystem.getNewTragetFiles());
        fileTable.setModel(fTableModel);
        fileTable.updateUI();
    }
}

/**
 * <p>Description: Template containing method-level mutation operators</p>
 * @version 1.0
 */
abstract class MOTableModel extends AbstractTableModel {
    private String[] columnHeader = new String[]{"", "Operator"};
    private String[] op;
    private Object[][] data;
...
class FileTableModel extends AbstractTableModel {
    ...
    private String[] columnHeader = new String[]{"", "Java   File"};
